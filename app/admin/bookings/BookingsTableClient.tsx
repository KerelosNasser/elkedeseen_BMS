              ...
              {filtered.map(b => (
                <div key={b.id} className="p-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="font-bold text-church-text">{b.title}</div>
                      <div className="text-xs text-church-text-muted mt-1">{b.venue.nameAr}</div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-y-2 text-xs border-t border-church-bg-warm pt-3">
                    <div className="text-church-text-muted">بواسطة:</div>
                    <div className="text-church-text font-semibold">{b.booker.name}</div>
                    <div className="text-church-text-muted">التاريخ:</div>
                    <div className="text-church-text">
                      {b.isRecurring ? "🔄 متكرر" : format(new Date(b.weekDate), "d MMM yyyy", { locale: ar })}
                    </div>
                    <div className="text-church-text-muted">الوقت:</div>
                    <div className="text-church-text">{formatTime(b.startTime)} - {formatTime(b.endTime)}</div>
                  </div>
                  ...
                  <div className="flex justify-end pt-2">
                    <button
                      onClick={() => handleDelete(b.id)}
                      disabled={isDeleting === b.id}
                      className="flex items-center gap-1 text-xs text-church-red bg-church-red/5 px-3 py-1.5 rounded-pill border border-church-red/10 font-bold"
                    >
